...
int audio = 0;
int audio_buffer_size = 4096; // samples (both left and right)
...
#define AUDIO_SAMPLES 1
#define AUDIO_SAMPLE_SIZE 2
#define AUDIO_CHANNELS 2
#define AUDIO_BUFFERS 10
#define AUDIO_BUFFER_SIZE (audio_buffer_size * 2) 
//(AUDIO_SAMPLE_SIZE*AUDIO_CHANNELS*audio_buffer_size)
...
void audio_write(double* buffer,int samples) {
    ...
    for(i=0;i<samples;i++) {
        ...
        short left_sample=(short)(buffer[source_index]*32768.0 * volume );
        short right_sample=(short)(buffer[source_index+1]*32768.0 * volume );
