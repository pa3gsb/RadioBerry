...
gint init(void* arg) {
  ...
  while(!start) {
	  ...
	  //pa3gsb.sn
		...
		discovered[devices].status=1;
		discovered[devices].device=1;
		discovered[devices].software_version= 1;
		discovered[devices].status = 1;
		discovered[devices].mac_address[0] = 1;
		discovered[devices].mac_address[1] = 2;
		discovered[devices].mac_address[2] = 3;
		discovered[devices].mac_address[3] = 4;
		discovered[devices].mac_address[4] = 5;
		discovered[devices].mac_address[5] = 6;
		strcpy(discovered[devices].interface_name,"rb");
      ...
      if(devices==0) {
          ...
          for(i=0;i<devices;i++) {
              ...
              sprintf(text,"%s (%s %d.%d) %s (%02X:%02X:%02X:%02X:%02X:%02X) on %s\n",
                        d->name,
                        d->protocol==ORIGINAL_PROTOCOL?"radioberry":"new",
  ...
  if (d->protocol==RADIOBERRY_PROTOCOL) {
