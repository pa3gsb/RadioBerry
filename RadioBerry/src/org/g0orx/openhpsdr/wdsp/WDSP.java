...
public class WDSP {
    ...
    // Note that f_low must always be numerically less than f_high.  (default f_low = â€?4150.0; default f_high = â€?
    // 150.0)
    ...
    public native void SetRXAFMDeviation(int channel, double deviation);
        ...
        // Deviation is in Hertz.  Internal defaults exist for PLL settings and deâ€?emphasis; these have not currently
    // been made externally available â€?â€? they could be.
    ...
    public static final int LONG = 1; // (hangtime = 2000ms, Ï„_decay = 2000ms)
    public static final int SLOW = 2; // (hangtime = 1000ms, Ï„_decay = 500ms)
    public static final int MED = 3; //  (No Hang, Ï„_decay = 250ms)
    public static final int FAST = 4; //  (No Hang, Ï„_decay = 50ms)
    ...
    // Get position data for the 'AGCâ€?T' line on the bandscope.

    public native void SetRXAAGCThresh(int channel, double thresh, double size, double rate);

    // 'thresh' is position data needed when the 'AGCâ€?T' line on the bandscope is moved.
    ...
    // 'setit' is 0 to turnâ€?off the carrier block and is '1' to turn it on.
    ...
    static {
        ...
        if(System.getProperty("os.name").startsWith("Windows")) {
           ...
           // Log.i("WDSP","load wdsp.dll");
            ...
            Log.info("WDSP","libraryPath: "+libraryPath);
            Log.info("WDSP","load libfftw3.so");
            System.load(libraryPath+File.separator+"libfftw3.so");
            Log.info("WDSP","load libwdsp.so");
            System.load(libraryPath+File.separator+"libwdsp.so");
            Log.info("WDSP","load libwdspj.so");
